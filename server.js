...
const TWILIO_SID   = (process.env.TWILIO_SID   || "ACfd288a5bd0a615c040078fb10f03f049").trim();
const TWILIO_TOKEN = (process.env.TWILIO_TOKEN  || "8b23ac5962d08ab9145784e68e274861").trim();
const TWILIO_FROM  = (process.env.TWILIO_FROM   || "whatsapp:+14155238886").trim();
const KENNETH_WA   = (process.env.KENNETH_WA    || "whatsapp:+919538864425").trim();
const FINNHUB_KEY  = (process.env.FINNHUB_KEY   || "d8r0mh9r01quatdats5gd8r0mh9r01quatdats60").trim();
...
} catch(e) { console.error("Twilio fejl:", e.message); }

const STOCKS = ["NVDA","MSFT","META","AMZN","TSM","CRWD","ASML","INFY"];

async function sendWA(msg) {
  if(!twilioClient) return;
  try { await twilioClient.messages.create({ from: TWILIO_FROM, to: KENNETH_WA, body: msg }); }
  catch(e) { console.error("WA fejl:", e.message); }
}
...
app.get("/test-wa", async (req,res) => {
  await sendWA("ROSVANG INVEST - TEST\nHarald backend er live!\n- Harald");
  res.json({ ok:true });
});
